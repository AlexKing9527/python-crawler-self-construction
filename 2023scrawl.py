...
def scrawlWeb ():
    url = 'https://www.women.org.cn/art/2024/3/4/art_180_175509.html'
    headers = getheaders()
    ...
    info = soup.find_all('div', class_='right')
    ...
    for i, val in enumerate(nameList):
        ...
        for index, name in enumerate(names_without_spaces):
            ...
            if (((len(val) < 9) & ("自治区" in val)) or ((len(val) < 4) & ("省" in val)) or ((len(val) < 4) & ("市" in val))):
                cityName = val
            if len(name) == 1:
                name = name + names_without_spaces[index + 1]
                names_without_spaces[index + 1] = ''
                data.append([warName, cityName, name, '2023'])
            else:
                if len(name) > 0:
                    data.append([warName, cityName, name, '2023'])
...
if __name__ == '__main__':
    ...
    scrawlWeb()
    # print(data)
    df = pd.DataFrame(data, columns = ['result', 'result2', 'result3', 'result4'])
